...
class RescaleT(object):
    ...
    def __call__(self, sample):
        ...
        lbl = transform.resize(label, (self.output_size, self.output_size), mode='constant', order=0,
                               preserve_range=True)
        ref = transform.resize(ref, (self.output_size, self.output_size), mode='constant')

        return {'image': img, 'label': lbl, 'reflection': ref}
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': image, 'label': label, 'reflection': ref}
...
class SalObjDataset(Dataset):
    def __init__(self, img_name_list, lbl_name_list, ref_name_list, transform=None):
        self.image_name_list = img_name_list
        self.label_name_list = lbl_name_list
        self.ref_name_list = ref_name_list
    ...
    def __getitem__(self, idx):
        ...
        if 0 == len(self.ref_name_list):
            ref = np.zeros(image.shape)
        else:
            ref = io.imread(self.ref_name_list[idx])
        ...
        if search:
            is_ref = 1.
        sample = {'image': image, 'label': label, 'reflection': ref}
